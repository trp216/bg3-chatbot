...
if "llm_model" not in st.session_state:
    st.session_state["llm_model"] = AutoModelWithLMHead.from_pretrained('output-medium')
...
if "messages" not in st.session_state:
    ...
    st.session_state.messages = [{"role": "assistant", "content": "Welcome, this is Nightwarden Minthara.","avatar":TQ_AVATAR}]
...
for step in range(10):
    ...
    data = tokenizer.decode(chat_history_ids[:, bot_input_ids.shape[-1]:][0], skip_special_tokens=True)
    ...
    if not data:
        ...
        try:
            ...
            print('name TQ_AVATAR is not defined')
    st.session_state.loading = False

    st.rerun()

st.session_state.messages = [{"role": "assistant", "content": "Your free trial has ended. Purchase the Premium subscription to keep chatting.","avatar":TQ_AVATAR}]